...
створенні об'єкта цього класу (наприклад Math.random()). Також цей клас повинен мати метод getSignature, який повертає
...
abstract class House {
  ...
  comeIn(person: Person): void {
    if (this.door) {
      ...
      console.log("The door is closed. Can not enter.");
    }
  }
...
class MyHouse extends House {
  ...
  openDoor(passedKey: Key) {
    if (this.key?.getSignature() === passedKey.getSignature()) {
      this.door = true;
